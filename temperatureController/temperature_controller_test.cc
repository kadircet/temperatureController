...
*/
TEST(Air_Conditioner_Test, SetTemperature) {
	MockTemperatureSensor* const pTemperatureSensor = new MockTemperatureSensor();
	MockFanSpeed* const pFanSpeed = new MockFanSpeed();
...
*/
TEST(Air_Conditioner_Test, CheckStatusWithFixedRoomTemp) {
	MockTemperatureSensor* const pTemperatureSensor = new MockTemperatureSensor();
	MockFanSpeed* const pFanSpeed = new MockFanSpeed();
	
	MockTemperatureController homeTempController(pFanSpeed, pTemperatureSensor);
	
	eFanSpeedSettings readFanSpeed = eFAN_SPEED_OFF;
	eCurrentStatus currentStatus = SYSTEM_OFF;
	...
	EXPECT_EQ(eFAN_SPEED_OFF, readFanSpeed);
	
	currentStatus = homeTempController.getCurrentStatus();
	EXPECT_EQ(SYSTEM_OFF, currentStatus);
	
	homeTempController.setSetTemperature(26);
	homeTempController.mainController();
	
	readFanSpeed = pFanSpeed->getFanSpeed();
	EXPECT_EQ(eFAN_SPEED_LOW, readFanSpeed);
	
	currentStatus = homeTempController.getCurrentStatus();
	EXPECT_EQ(HEATING_IN_PROGRESS, currentStatus);
	
	homeTempController.setSetTemperature(29);
	homeTempController.mainController();
	
	readFanSpeed = pFanSpeed->getFanSpeed();
	EXPECT_EQ(eFAN_SPEED_MID, readFanSpeed);
	...
	readFanSpeed = pFanSpeed->getFanSpeed();
	EXPECT_EQ(eFAN_SPEED_HIGH, readFanSpeed);
	...
	currentStatus = homeTempController.getCurrentStatus();
	EXPECT_EQ(COOLING_IN_PROGRESS, currentStatus);
...
*	Random tests are conducted.
*/
TEST(Air_Conditioner_Test, CheckStatusWithChangingRoomTemp) {
	MockTemperatureSensor* const pTemperatureSensor = new MockTemperatureSensor();
	MockFanSpeed* const pFanSpeed = new MockFanSpeed();
...
*	Random tests are conducted.
*/
TEST(Air_Conditioner_Test, CheckStatusWithChangingRoomTempWave) {
	MockTemperatureSensor* const pTemperatureSensor = new MockTemperatureSensor();
	MockFanSpeed* const pFanSpeed = new MockFanSpeed();
